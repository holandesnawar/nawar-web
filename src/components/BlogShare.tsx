...
export default function BlogShare({ title, variant = 'sidebar' }: { title: string; variant?: Variant }) {
  ...
  // Instagram no tiene URL pública de share. Estrategia:
  // - Mobile con Web Share API → abre el share sheet nativo (Instagram,
  //   Stories, DM, etc.)
  // - Desktop → copia el enlace y abre instagram.com en pestaña nueva
  const shareInstagram = async () => {
    const shareData = { title: `${title} — Holandés Nawar`, url }
    if (typeof navigator !== 'undefined' && (navigator as any).share) {
      try {
        await (navigator as any).share(shareData)
        return
      } catch {
        /* usuario canceló — caemos al fallback */
      }
    }
    // Fallback: copia + abre Instagram
    try {
  ...
  return (
    <div className={wrapperClass}>
      ...
      {/* Instagram — share por Web Share API (mobile) o copy link + abrir IG (desktop) */}
      <button
        onClick={shareInstagram}
        title={copied ? 'Enlace copiado — pégalo en Instagram' : 'Compartir en Instagram'}
